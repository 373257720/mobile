<template>
	<div id="mhome">
		<header>
			...
			<van-search v-model="searchkey" :placeholder="$t('common.PleaseEnterTheSearchKeyword')" show-action shape="round"
			 left-icon @search="onSearch">
			...
			<van-dropdown-menu>
				<van-dropdown-item :title="industry_title" v-model="industry_value" ref="item">
					<van-tree-select :items="items" :active-id="activeIds" :main-active-index="mainActiveIndex" @click-nav="onClickNav"
				...
				<van-dropdown-item class='region_class' :title="region_title" ref="region">
					...
					<ul v-if='!countrylist_fetching && countrylist.length>0'>
						<li v-for="d in countrylist" :class="d.classname" :key="d.remark" :value='d.value' @click="select_country(d.remark,d.eng,d.chinese,d.value)">
							<span>{{d.eng}}</span>
							<span>{{d.chinese}}</span>
						</li>
					</ul>
		...
		<div id="main">
			<div class="main">
				<van-pull-refresh v-model="refreshing" @refresh="onRefresh">
					<van-list v-model="loading" :finished="finished" @load="onLoad" :loading-text="loadText" :finished-text="$t('common.NoMore')"
					 :error-text="$t('common.RequestFailed')" :offset="300">
						<div v-for="(goods,idx) in  upGoodsInfo" :key="idx" class="goodlists">
								...
								<div class="tag" v-if="usertype==1">
									<ul>
										<li v-for="(item,key) in  tags" :key="item.text">
											<div v-if="goods.signUserList[key].length>0 && goods.signUserList[key][0].signCount">
							...
							<footer>
								<button v-if="usertype==1" :class="[goods.signUserList['signUserList10'][0].signCount>0?'isactive':'']" @click="router('p_investor_lists',{arr: JSON.stringify(goods.signUserList['signUserList10'][0].investorsIdList) })">{{$t('common.InformationOfContractedInvestors')}}
									({{goods.signUserList['signUserList10'][0].signCount?goods.signUserList['signUserList10'][0].signCount:0}})
								</button>
								<button class="isactive" v-else-if="usertype==3" @click="$routerto('i_conected_project',{projectId:goods.projectId,signStatus:goods.signUserResp[0].signStatus,signId:goods.signUserResp[0].signId})">{{$t('common.ConnectedItems')}}</button>
