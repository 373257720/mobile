  ...
  <div id="a_bargin">
    ...
    <main>
      <div class="bargin-upper">
        <h3>{{$t('Bargin.Projectparty')}} （用戶名）{{$t('Bargin.Suggest')}}</h3>
      ...
      <div class="bargin-lower">
        <h3>{{$t('Bargin.Intermediary')}} （用戶名）{{$t('Bargin.SuggestBack')}}</h3>
        <div v-if="obj.sharingMechanismType4===0">
          <p>{{$t('Bargin.Percentagebyintermediaries')}}</p>
          <div class="count">
            <MyNumberInput
        ...
        <ul v-if="obj.sharingResult==2">
          ...
          <li>
            ...
            <van-button @click="pick(1)" class="renewal">{{$t('Bargin.Suggest')}}</van-button>
          </li>
          <li>
  ...
  methods: {
    ...
    acceptOrRejectCommission(num) {
      ...
      if (this.obj.sharingMechanismType4 === 1) {
        sharingMechanism0 = this.obj.sharingMechanism11;
